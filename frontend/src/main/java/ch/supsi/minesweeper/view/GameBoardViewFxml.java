...
public class GameBoardViewFxml implements ControlledFxView {
    ...
    public void initialize(EventHandler eventHandler, AbstractModel model) {
        ...
        this.boardModel = (BoardModel) model;
    }
    ...
    private void createMatrix(int rows, int cols) {
        ...
        containerPane = new GridPane();
        containerPane.setPadding(new Insets(30));
        containerPane.setHgap(1);
        containerPane.setVgap(1);
    ...
    private void updateButton(int row, int col) {
        ...
        if (tile.isUncovered()) {
            if (tile.isBomb()) {
                FontIcon icon = new FontIcon(FontAwesomeSolid.BOMB);
                icon.setIconSize(14);
            ...
            FontIcon icon = new FontIcon(FontAwesomeSolid.FLAG);
            icon.setIconSize(14);
            icon.setIconColor(Paint.valueOf("white"));
            button.setGraphic(icon);
        } else {
